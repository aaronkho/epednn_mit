...
def generate_pytorch_scoping_model(config, params):
    ...
    model = EPEDNN_Scoping(layers, **config)
    if params:
        with torch.no_grad():
...
def generate_epednn_mit_scoping_pytorch(weights_list):
    n_inputs = 1
    n_outputs = 1
    model_list = []
    for i, weights_dict in enumerate(weights_list):
        config, params = configure_weights_for_pytorch(weights_dict)
        model_list.append(generate_pytorch_scoping_model(config, params))
...
def test_epednn_mit_scoping_pytorch():
    test_in = np.atleast_2d([1.3, 3.0, 1.9, 0.5, 8.7, 4.0, 2.0, 2.5, 0.75, 0.5, 300.0]).astype(np_dtype)
    test_out = np.atleast_2d([3.5654504e+02, 5.6555986e-02]).astype(np_dtype)
    root = Path(__file__).resolve().parent
    weights_files = [Path(f).resolve() for f in sorted(root.glob('*scoping*.pkl'))]
    weights_list = load_weights(weights_files)
    model = generate_epednn_mit_scoping_pytorch(weights_list)
