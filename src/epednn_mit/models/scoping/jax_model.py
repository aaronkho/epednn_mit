...
class EPEDNNmit(nn.Module):
    """A single member of the EPEDNN-mit scoping model ensemble.
    ...
    hidden_dims: Final[Sequence[int]] = (200, 100, 50)
...
class EPEDNNmitEnsemble(nn.Module):
    """An ensemble of EPEDNN-mit scoping models.
...
def test_epednn_mit_scoping_jax():
    """Tests the EPEDNNmitEnsemble scoping model loading and inference.
    ...
    """
    test_in = jnp.atleast_2d(
        jnp.array([1.3, 3.0, 1.9, 0.5, 8.7, 4.0, 2.0, 2.5, 0.75, 0.5, 300.0])
    )
    test_out = jnp.atleast_2d(jnp.array([3.5654504e02, 5.6555986e-02]))
